...
int findMenorRecursive (ArrayPokemon *array, int currentIndex, int minIndex) 
{
//Condicao de parada
  if (currentIndex >= array->length) return minIndex;  
//Comparar strings
  if (strcasecmp(array->pks[currentIndex]->name, array->pks[minIndex]->name) < 0)
    minIndex = currentIndex; 
  comparacoes++;
//Retornar
  return findMenorRecursive (array, currentIndex + 1, minIndex);
}

void selectionSortRecursive (ArrayPokemon * array, int i)
{
//Definir dados locais
  int n = array->length;
//Condicao de parada
  if (i < array->length)
  {
  //Procurar pelo index do menor numero
    int menor = findMenorRecursive (array, i + 1, i);
  //Trocar de posicao
    Pokemon * tmp = array->pks[menor];
...
void aux_selectionSortRecursive (ArrayPokemon * array)
{
//Testar apontador
  if (array == NULL) return;
//Chamar funcao propriamente dita
