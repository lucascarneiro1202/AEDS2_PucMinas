...
void selectionSort (ArrayPokemon * array)
{
//Definir dados locais
  int n = array->length;
//Algoritmo de ordenação
  for (int i = 0; i < (n - 1); i++) 
  {
    ...
    for (int j = (i + 1); j < n; j++)
    {
      if (strcasecmp(array->pks[menor]->name, array->pks[j]->name) > 0) menor = j;
      comparacoes++;
    }
    Pokemon * temp = array->pks[menor];
    array->pks[menor] = array->pks[i];
    array->pks[i] = temp;
    movimentacoes += 3;
  }
}
...
void trim (char * str)
{
//Contar caracteres diferentes de espaco em branco
  int count = 0;
//Percorrer string
  for (int i = 0; str[i]; i++)
...
void countingSort (ArrayPokemon * array, int index, char lower, char upper)
{
//Testar ponteiro
  if (array == NULL) return;
//Definir tamanho do arranjo
  int n = array->length;
//Definir arranjo de contagem
  int * countArray = (int *) calloc ( (upper - lower + 2), sizeof(int) );
  if (countArray == NULL) return;
//Definir array de pokemon temporario
  ArrayPokemon * tempArray = new_ArrayPokemon(array->size);
  if (tempArray == NULL) {  free(countArray); return;  }
//Contar os caracteres de cada string no index especificado
  for (int i = 0; i < n; i++)
  {
    char * ability = ArrayList_get( array->pks[i]->abilities, 0 );
    char * trimmed = strdup(ability);
    trim(trimmed);
    int charIndex = (strlen(trimmed) - 1 < index) ? 0 : ((tolower(trimmed[index]) - lower) + 1);
    countArray[charIndex]++;
    free(trimmed);
  }
//Fazer a soma acumulativa no array de contagem
  int countArrayLength = (upper - lower + 2);
  for (int i = 1; i < countArrayLength; i++)
    countArray[i] += countArray[i - 1];
//Preencher arranjo temporario
  for (int i = n - 1; i >= 0; i--)
  {
    char * ability = ArrayList_get(array->pks[i]->abilities, 0);
    char * trimmed = strdup(ability);
    trim(trimmed);
    int charIndex = ( strlen(trimmed) - 1 < index ) ? 0 : (tolower(trimmed[index]) - lower) + 1;
    ArrayPokemon_set (tempArray, countArray[charIndex] - 1, array->pks[i]);
    countArray[charIndex]--;
    movimentacoes++;
  }
//Copiar elementos
  for (int i = 0; i < n; i++)
  {  ArrayPokemon_set (array, i, tempArray->pks[i]); movimentacoes++;  } 
//Desalocar memoria
  free(countArray);
  free_ArrayPokemon(tempArray);
}

void radixSort (ArrayPokemon * array, char lower, char upper)
{
//Testar ponteiro
  if (array == NULL) return;
//Procurar o maior valor
  ...
  for (int i = 0; i < array->length; i++)
  {
    ...
    char * trimmed = strdup(ability);
    trim(trimmed);
    if (strlen(trimmed) - 1 > maxIndex) maxIndex = strlen(trimmed) - 1;
    free(trimmed);
  }
//Realizar as chamadas do counting sort
...
//Escrever arquivo de resposta
  FILE * file = fopen ("857340_radix.txt", "wt");
  if (file != NULL)
