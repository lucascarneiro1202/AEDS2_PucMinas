...
{
//Testar ponteiros
  if (date1 == NULL || date2 == NULL) return -1000;
//Dividir as datas
  char ** splittedDate1 = split(date1, '/', 3, 5);
  if (splittedDate1 == NULL)
    return -1000;
  char ** splittedDate2 = split(date2, '/', 3, 5);
  if (splittedDate2 == NULL)
  {
    for (int i = 0; i < 3; i++) free(splittedDate1[i]);
    free(splittedDate1);
    return -1000;
  }
//Comparar
  int res = 0;
  res = strcasecmp ( splittedDate1[2], splittedDate2[2] );
  if (res == 0)
  {
    res = strcasecmp ( splittedDate1[1], splittedDate2[1] );
    if (res == 0)
      res = strcasecmp ( splittedDate1[0], splittedDate2[0] );
  }
//Desalocar memoria
  for (int i = 0; i < 3; i++)
  {
    free(splittedDate1[i]);
    free(splittedDate2[i]);
  }
  free(splittedDate1);
  free(splittedDate2);
//Retornar
...
void insertionSort (ArrayPokemon * array)
{
//Testar ponteiro
...
//Algoritmo de ordenacao
  for (int i = 1; i < n	; i++) 
  {
    ...
    movimentacoes++;
    int j = i - 1;
    while ( (j >= 0) && ( compareDate(Date_format(array->pks[j]->captureDate), Date_format(tmp->captureDate)) > 0 ) ) 
    {
      //array[j] > tmp
       array->pks[j + 1] = array->pks[j];
       j--;
       comparacoes++;
       movimentacoes++;
    }
