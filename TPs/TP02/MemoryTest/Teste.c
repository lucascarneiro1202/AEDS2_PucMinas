...
//Testar ponteiro
  if (array == NULL || array->pks == NULL) 
    return;
//Desalocar memoria
  for (int i = 0; i < array->size; i++) free_Pokemon(array->pks[i]);
  free(array->pks);
  free(array);
}
...
//Adicionar elemento
  free_Pokemon(array->pks[array->length]);
  array->pks[array->length] = Pokemon_clone(pk);
  array->length++;
}

void ArrayPokemon_set (ArrayPokemon * array, int index, Pokemon * pk)
{
//Testar condicoes
  if (array == NULL || index >= array->size || pk == NULL) return;
//Adicionar elemento
  free(array->pks[index]);
  array->pks[index] = Pokemon_clone(pk);
...
void swap (ArrayPokemon * array, int i, int j)
{
//Testar ponteiro e posicoes
  if (array == NULL || i < 0 || i > array->length || j < 0 || j > array->length)
    return;
//Trocar de posicao
  Pokemon * tmp = array->pks[i];
  array->pks[i] = array->pks[j];
  array->pks[j] = tmp;
  tmp = NULL;
}

void construir (ArrayPokemon * array, int tamHeap)
{
//Testar ponteiro
  if (array == NULL || tamHeap < 0) return;
//Construir arvore heap invertida
  for (int i = tamHeap; i > 1; i /= 2)
  {
  //Definir dados
    Pokemon * child = array->pks[i];
    Pokemon * parent = array->pks[i / 2];
  //Testar se o filho e' maior que o pai
    ...
    {
      if (child->height == parent->height && strcasecmp(child->name, parent->name) > 0)
      {
        swap(array, i, i/2);
        comparacoes += 2;
        movimentacoes += 3;
...
int getMaiorFilho (ArrayPokemon * array, int i, int tamHeap)
{
//Testar ponteiro
  if (array == NULL || array->pks == NULL) return -1;
//Definir dados locais
  int filho;
//Testar qual e' o filho
  if (2 * i == tamHeap || array->pks[2 * i]->height > array->pks[2 * i + 1]->height)
  {  
    filho = 2 * i; 
    comparacoes++;  
  }
  ...
  {
    if (array->pks[2 * i]->height == array->pks[2 * i + 1]->height)
    {
      filho = 2 * i;
      if ( strcasecmp(array->pks[2 * i]->name, array->pks[2 * i + 1]->name) < 0 ) filho = 2 * i + 1;
      comparacoes += 2;
    }
    else
    {  
      filho = 2 * i + 1; comparacoes++;  
    }
  }
//Retornar
  return filho;
...
void reconstruir (ArrayPokemon * array, int tamHeap)
{
//Testar ponteiro
...
//Reconstruir heap
  while (i <= tamHeap / 2)
  {
    ...
    if (array->pks[i]->height < array->pks[filho]->height)
    {
      swap(array, i, filho);
      i = filho;
      comparacoes++;
    ...
    {
      if (array->pks[i]->height == array->pks[filho]->height && strcasecmp(array->pks[i]->name, array->pks[filho]->name) < 0) 
      {
        swap(array, i, filho);
        i = filho;
        comparacoes += 2;
...
void heapSort (ArrayPokemon * array)
{
//Testar ponteiro
 if (array == NULL || array->pks == NULL) return;
//Definir dados locais
  int n = array->length;
//Definir novo arranjo com index a partir do 1
  ArrayPokemon * arrayTmp = new_ArrayPokemon(array->size);
//Preencher arranjo temporario com index a partir do 1
  for (int i = 0; i < n; i++)
    ArrayPokemon_set (arrayTmp, i + 1, array->pks[i]);
//Construir o heap invertido
  for (int tamHeap = 2; tamHeap <= n; tamHeap++)
    construir (arrayTmp, tamHeap);
//OrGdenacao propriamente dita
  int tamHeap = n;
  while (tamHeap > 1)
  {
...
//Recolocar valores para index 0
  for (int i = 0; i < n; i++)
    ArrayPokemon_set (array, i, arrayTmp->pks[i + 1]);
//Desalocar memoria do arranjo temporario
  free_ArrayPokemon (arrayTmp);
}
