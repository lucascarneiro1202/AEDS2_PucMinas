...
//Definir dados locais
  ...
  for (int i = 0; i < (n - 1); i++) 
  {
    ...
    for (int j = (i + 1); j < n; j++)
    {
      if ( strcasecmp( array->pks[menor]->name, array->pks[j]->name ) > 0 )
        menor = j;
      comparacoes++;
    }
...
bool pesquisaBinaria (ArrayPokemon * array, char * name)
{
//Testar condicoes
  if (array == NULL || array->pks == NULL || name == NULL)
    return false;
  for (int i = 0; i < array->length; i++)
    if (array->pks[i] == NULL) return false;
//Definir dados locais
  bool resp = false;
...
//Algoritmo de pesquisa
  ...
  {
    meio = (esq + dir) / 2;
    if ( strcasecmp(name, array->pks[meio]->name) == 0 )
    {  resp = true; esq = dir + 1;  }
    else
    {
      if ( strcasecmp(name, array->pks[meio]->name) > 0 )
...
//Ler elementos
  ...
  {
  //Calcular resultado
    bool resp = pesquisaBinaria (array, line);
  //Mostrar resultado
    printf("%s\n", (resp) ? "SIM" : "NAO");
  //Ler proxima linha  
    free(line);
...
//Escrever arquivo de resposta
  FILE * file = fopen ("857340_binaria.txt", "wt");
  if (file != NULL)
