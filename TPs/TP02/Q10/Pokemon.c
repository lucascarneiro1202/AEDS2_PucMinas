...
void quicksortRec(ArrayPokemon * array, int esq, int dir) 
{
  int i = esq, j = dir;
  Pokemon * pivo = array->pks[(dir + esq) / 2];
  while (i <= j) 
  {
    while (array->pks[i]->generation < pivo->generation || (array->pks[i]->generation == pivo->generation && strcasecmp( array->pks[i]->name, pivo->name ) < 0)) 
    {  i++; comparacoes++;  }
    while (array->pks[j]->generation > pivo->generation || (array->pks[j]->generation == pivo->generation && strcasecmp( array->pks[j]->name, pivo->name ) > 0) ) 
    {  j--; comparacoes++;  }
    if (i <= j) 
    {
...
void quickSort(ArrayPokemon * array)
{  
//Testar apontador
...
//Escrever arquivo de resposta
  FILE * file = fopen ("857340_quicksort.txt", "wt");
  if (file != NULL)
