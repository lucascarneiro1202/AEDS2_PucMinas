...
void insercaoPorCor (ArrayPokemon * array, int n, int cor, int h)
{
  ...
  for (int i = (h + cor); i < n; i += h) 
  {
    ...
    movimentacoes++;
    int j = i - h;
    while ((j >= 0) && ( ((array->pks[j]->weight > tmp->weight)) || (array->pks[j]->weight == tmp->weight && strcasecmp( array->pks[j]->name, tmp->name ) > 0) )) 
    {
      array->pks[j + h] = array->pks[j];
      j -= h;
      comparacoes++;
      movimentacoes++;
    }
...
void shellSort (ArrayPokemon * array)
{
//Testar apontador
...
//Algoritmo de ordenacao
  int h = 1;
  do { h = (h * 3) + 1; } while (h < n); //Encontrar o 'h' adequado
  do 
  {
    h /= 3;
...
//Escrever arquivo de resposta
  FILE * file = fopen ("857340_shellsort.txt", "wt");
  if (file != NULL)
