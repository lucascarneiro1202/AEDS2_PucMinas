...
/* --------------------------- DEFINIR ARVORE BINARIA DE POKEMON E SEUS METODOS --------------------------- */
...
No  * new_No (char * nomePokemon) {
//Definir no´
  No * no = malloc(sizeof(No));
  if (no == NULL) return NULL;
//Preencher dados do no´
  no->nomePokemon = strdup(nomePokemon);
  no->esq = NULL;
  no->dir = NULL;
  no->nivel = 1;
//Retornar
...
//Testar no'
  if (no == NULL) return;
//Desalocar memorias
  if (no->nomePokemon != NULL) free(no->nomePokemon);
  no->esq = NULL;
  no->dir = NULL;
  free(no);
}
...
//Testar no´
  if (no == NULL) return;
//Atualizar o nivel do no
  no->nivel = 1 + ( (getNivel(no->esq) > getNivel(no->dir) ? getNivel(no->esq) : getNivel(no->dir) ) );
}

typedef struct ArvoreBinariaAVL {
  struct No * raiz;
} ArvoreBinariaAVL;

ArvoreBinariaAVL * new_ArvoreBinariaAVL () {
//Definir arvore binaria
  ArvoreBinariaAVL * arvore = malloc(sizeof(ArvoreBinariaAVL));
//Testar alocacao
  if (arvore == NULL) return NULL;
//Definir dados iniciais da arvore
  arvore->raiz = NULL;
//Retornar
  return arvore;
}

void free_ArvoreBinariaAVL (ArvoreBinariaAVL * arvore) {
//Testar arvore
  if (arvore == NULL) return;
//Função recursiva para desalocar todos os nos
  void free_NoRec(No * no) {
  //Testar o no'
    if (no == NULL) return;
  //Desalocar subarvores
    free_NoRec(no->esq);
    free_NoRec(no->dir);
  //Liberar memória do no' atual
    free_No(no);
  }
//Liberar nos da arvore
  free_NoRec(arvore->raiz);
//Liberar estrutura da arvore
  free(arvore);
}


bool pesquisarRec (char * nomePokemon, No * no) {
//Testar nome do pokemon
  if (nomePokemon == NULL) return false;
//Testar dados
  bool resp;
  if (no == NULL) {
    resp = false;
  } else if (strcmp(nomePokemon, no->nomePokemon) == 0) {
    comparacoes++;
    resp = true;
  } else if (strcmp(nomePokemon, no->nomePokemon) < 0) {
    comparacoes++;
    printf("esq ");
    resp = pesquisarRec(nomePokemon, no->esq);
  } else {
    comparacoes++;
    printf("dir ");
    resp = pesquisarRec(nomePokemon, no->dir);
  }
//Retornar
  return resp;
}

bool pesquisar (ArvoreBinariaAVL * arvore, char * nomePokemon) {
  if (arvore == NULL || nomePokemon == NULL) return false;
  printf("raiz ");
  return pesquisarRec(nomePokemon, arvore->raiz);
}

No * rotacionarDir (No * no) {
//Testar no'
  if (no == NULL || no->esq == NULL) return NULL;
//Definir dados locais
  No * noEsq = no->esq;
...
//Testar no'
  if (no == NULL || no->dir == NULL) return NULL;
//Definir dados locais
  No * noDir = no->dir;
...
//Testar no'
  ...
  //Testar se esta' balanceada
    if (abs(fator) <= 1) {
      setNivel(no);
    } else if (fator == 2) { //Testar se esta' desbalanceada para a direita
    //Calular o fator do filho 'a direita
      int fatorFilhoDir = getNivel(no->dir->dir) - getNivel(no->dir->esq);
    //Testar se o filho da direita tambem esta desbalanceado
      if (fatorFilhoDir == -1) {
        no->dir = rotacionarDir(no->dir); //Rotacao dir-esq
      }
    //Rotacionar para a esquerda
      no = rotacionarEsq(no);
    } else if (fator == -2) { //Testar se esta' desbalanceada para a esquerda
    //Calular o fator do filho 'a esquerda
      int fatorFilhoEsq = getNivel(no->esq->dir) - getNivel(no->esq->esq);
    //Testar se o filho da esquerda tambem esta desbalanceado
      if (fatorFilhoEsq == 1) {
        no->esq = rotacionarEsq(no->esq); //Rotacao esq-dir
      }
    //Rotacionar para a direita
      no = rotacionarDir(no);
    } else { //Se houver erro
      printf("[ERRO]: Erro no balanceamento do No (%s) com o fator (%d) invalido!\n", no->nomePokemon, (getNivel(no->dir) - getNivel(no->esq)));
    }    
  }
//Retornar
  return no;
...
No * inserirRec (char * nomePokemon, No * no) {
//Testar nome do pokemon
  if (nomePokemon == NULL) return NULL;
//Testar dados
  if (no == NULL) {
    no = new_No(nomePokemon);
  } else if (strcmp(nomePokemon, no->nomePokemon) < 0) {
    comparacoes++;
    no->esq = inserirRec(nomePokemon, no->esq);
  } else if (strcmp(nomePokemon, no->nomePokemon) > 0) {
    comparacoes++;
    no->dir = inserirRec(nomePokemon, no->dir);
  } else {
...
No * inserir (char * nomePokemon, ArvoreBinariaAVL * arvore) {
//Testar ponteiros
  if (arvore == NULL || nomePokemon == NULL) return NULL;
//Chamar metodo recursivo
  arvore->raiz = inserirRec(nomePokemon, arvore->raiz);
//Retornar
  return arvore->raiz;
}
...
//Definir arvore binaria AVL
  ArvoreBinariaAVL * arvore = new_ArvoreBinariaAVL();
  if (arvore == NULL) {  free(allContent); return -1;  }
...
//Laco de repeticao
  ...
  //Testar se string e' um numero
    ...
    //Adicionar 'a arvore binaria
      inserir(pokemon->name, arvore);
    //Desalocar memoria
...
//Escrever arquivo de resposta
  FILE * file = fopen("857340_avl.txt", "wt");
  if (file != NULL)
    fprintf(file, "857340\t%lld\t%d", (fim - inicio), comparacoes);
  fclose(file);
//Desalocar memorias
  free_ArvoreBinariaAVL(arvore);
  free(line);
  free(allContent);
  for (int i = 0; i < MAX_TAM_NUM_LINES; i++)
    free(allLines[i]);
