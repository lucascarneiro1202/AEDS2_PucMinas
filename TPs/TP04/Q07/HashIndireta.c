...
bool pesquisar (Lista * lista, char * nomePokemon) {
//Testar apontadores
  if (lista == NULL || nomePokemon == NULL) return false;
//Definir dados locais
  Celula * i = lista->primeiro->prox;
  bool resp = true;
  while (i != NULL && resp) {
    resp = strcmp(i->pokemon->name, nomePokemon) != 0;
    i = i->prox;
    comparacoes++;
  }
//Retornar
...
int calcularSomaASCII(char * palavra) {
//Testar palavras nulas
...
int hash(char * nomePokemon) {
  return calcularSomaASCII(nomePokemon) % 21;
}

void inserirTabelaHash(Lista ** tabelaHash, Pokemon * pokemon) {
//Testar apontadores
  if (tabelaHash == NULL || pokemon == NULL) return;
//Calcular o hash
  int hashValue = hash(pokemon->name);
//Inserir na tabela hash
  inserirFim(tabelaHash[hashValue], pokemon);
}

int pesquisarTabelaHash(Lista ** tabelaHash, char * nomePokemon) {
//Testar apontadores
  if (tabelaHash == NULL || nomePokemon == NULL) return -1;
//Definir dados locais
  int resp;
  int hashValue = hash(nomePokemon);
//Testar se a posicao esta ocupada
  comparacoes++;
  if (tabelaHash[hashValue]->length > 0 && pesquisar(tabelaHash[hashValue], nomePokemon)) {
    resp = hashValue;
  } else {
    resp = -1;
  }
...
//Definir tabela hash
  Lista ** tabelaHash = malloc(21 * sizeof(Lista *));
  for (int i = 0; i < 21; i++) {
    tabelaHash[i] = new_Lista();
  }
...
//Laco de repeticao
  ...
  //Testar se string e' um numero
    ...
    //Adicionar 'a tabela hash
      inserirTabelaHash(tabelaHash, pokemon);
    //Desalocar memoria
      free_Pokemon(pokemon);
    }
  //Ler proxima linha
...
//Leitura de todos os pokemons a serem pesquisados
  ...
  //Mostrar resultado
    int resp;
    printf("=> %s: ", line);
    resp = pesquisarTabelaHash(tabelaHash, line);
    if (resp != -1) {
...
//Escrever arquivo de resposta
  FILE * file = fopen("857340_hashIndireta.txt", "wt");
  if (file != NULL)
    fprintf(file, "857340\t%lld\t%d", (fim - inicio), comparacoes);
  fclose(file);
//Desalocar memorias
  for (int i = 0; i < 21; i++) {
    free_Lista(tabelaHash[i]);
  }
  free(tabelaHash);
  free(line);
  free(allContent);
