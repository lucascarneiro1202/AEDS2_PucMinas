...
/* --------------------------- DEFINIR Pilha DE POKEMON E SEUS METODOS --------------------------- */
...
//Definir dados iniciais
  pilha->primeiro = new_CelulaVazia();
  if (pilha->primeiro == NULL) {  free(pilha);  return NULL;  }
...
//Desalocar memoria
  ...
  free_Celula(celula);
  i = pilha->ultimo->prox = NULL;
//Retornar
  return pokemon;
}

void mostrarPilhaRec(Pilha * pilha, int i) {
  if (pilha->length > 0) {
