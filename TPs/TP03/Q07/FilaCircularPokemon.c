...
//Definir dados iniciais
  fila->primeiro = new_CelulaVazia();
  if (fila->primeiro == NULL) {  free(fila);  return NULL;  }
...
//Separar elemento a ser removido
  Celula * celula = fila->ultimo;
//Logica de ponteiros
  Celula * i;
  for (i = fila->primeiro; i->prox != fila->ultimo; i = i->prox);
  fila->ultimo = i;
//Diminuir o tamanho
  fila->length--;
//Desalocar memoria
  ...
  free_Celula(celula);
  i = fila->ultimo->prox = NULL;
//Retornar
  return pokemon;
}

void inserirInicio (Fila * fila, Pokemon * pokemon) {
//Testar fila
  if (fila == NULL)
  {    printf("[ERRO]: Fila invalida.\n"); return;   }
...
//Testar quantidade de elementos
  if (fila->length >= 5)
  {   free_Pokemon( removerFim(fila) );    }
//Definir nova celula
  Celula * novaCelula = new_Celula(pokemon);
//Definir logica de ponteiros
  novaCelula->prox = fila->primeiro->prox;
  fila->primeiro->prox = novaCelula;
  if (fila->primeiro == fila->ultimo) fila->ultimo = novaCelula;
  novaCelula = NULL;
//Aumentar o tamanho
  fila->length++;
}
...
//Somar todos os elementos
  Celula * cel = fila->primeiro->prox;
  int soma = 0;
  for (int i = 0; i < fila->length; i++) {
    soma += cel->pokemon->captureRate;
    cel = cel->prox;
  }
//Calcular a media
